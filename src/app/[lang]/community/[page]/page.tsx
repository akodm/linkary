...
const getMetadata = async (lang: string, page: string): Promise<Metadata> => {
  const manifest = await import(`public/manifest-${lang}.json`);
  const baseUrl = NEXT_PUBLIC_VERCEL_URL;
  const currentPage = Number(page);
  const currentUrl = `${baseUrl}/${lang}/community/${currentPage}`;
  ...
  > = {
    ko: {
      title: (page) => `커뮤니티 - ${page}페이지 | Linkary`,
      description:
    ...
    en: {
      title: (page) => `Community - Page ${page} | Linkary`,
      description:
    ...
    jp: {
      title: (page) => `コミュニティ - ${page}ページ | Linkary`,
      description:
    ...
    cn: {
      title: (page) => `社区 - 第${page}页 | Linkary`,
      description:
  ...
  return {
    title: pageMeta.title(currentPage),
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: pageMeta.title(currentPage),
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      ...
      languages: {
        en: `${baseUrl}/en/community/${currentPage}`,
        ko: `${baseUrl}/ko/community/${currentPage}`,
        ja: `${baseUrl}/jp/community/${currentPage}`,
        zh: `${baseUrl}/cn/community/${currentPage}`,
      },
    },
  };
};
